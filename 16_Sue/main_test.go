...
func TestRun(t *testing.T) {
	for _, test := range test {
		if actual := run(test.input, test.known, false); actual != test.expected {
...
func TestRunAdvanced(t *testing.T) {
	for _, test := range test2 {
		if actual := run(test.input, test.known, true); actual != test.expected {
			t.Errorf("Parse(%s) = %d, expected %d.",
				test.input, actual, test.expected)
		}
	}
...
const day16data = `Sue 1: goldfish: 9, cars: 0, samoyeds: 9
Sue 2: perfumes: 5, trees: 8, goldfish: 8
