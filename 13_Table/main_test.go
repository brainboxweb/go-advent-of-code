...
func TestRun(t *testing.T) {
	for _, test := range test2 {
		if actual := Run(test.input, false); actual != test.expected {
...
func TestRunWithGuest(t *testing.T) {
	for _, test := range test3 {
		if actual := Run(test.input, true); actual != test.expected {
			t.Errorf("Convert(%s) = %d, expected %d.",
				test.input, actual, test.expected)
		}
	}
...
const day13data = `Alice would gain 2 happiness units by sitting next to Bob.
