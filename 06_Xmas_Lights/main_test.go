...
func TestToggle(t *testing.T) {
	...
	toggle(lights, key) //Should switch off
	_, ok := lights[key]
	expected := false
	require.Equal(t, expected, ok)

	toggle(lights, key) //Should switch on
	_, ok2 := lights[key]
	expected2 := true
...
func TestProcesslight(t *testing.T) {
	...
	processlight(lights, key, "off", adjustedInstructions) //Should switch off
	_, ok := lights[key]
	expected := false
	require.Equal(t, expected, ok)

	processlight(lights, key, "on", adjustedInstructions) //Should switch on
	_, ok2 := lights[key]
	expected2 := true
	require.Equal(t, expected2, ok2)

	processlight(lights, key, "toggle", adjustedInstructions) //Should switch off
	_, ok3 := lights[key]
	expected3 := false
...
func TestInstruction(t *testing.T) {
	...
	adjustedInstructions := false
	processInstruction(lights, adjustedInstructions, input)
...
func TestGetLightCount(t *testing.T) {
	for _, test := range tests2 {
		if actual := GetLightCount(test.input); actual != test.expected {
...
func TestSwitchOnAdjusted(t *testing.T) {
	...
	require.Equal(t, expected, lights[key])

	switchOnAdjusted(lights, key)  //Switch on again
	expected2 := light{Level: 2}
	require.Equal(t, expected2, lights[key])
...
func TestSwitchOffAdjusted(t *testing.T) {
	...
	var lights = make(map[string]light)
	key := "0,0"
	lights[key] = light{Level: 2}

	switchOffAdjusted(lights, key)
	expected :=  light{Level: 1}
	require.Equal(t, expected, lights[key])

	switchOffAdjusted(lights, key) //should delete it
...
var tests3 = []struct {
	...
	{"turn on 0,0 through 999,999", 2000000},
	{day6data, 14687245},
}

func TestTotalBrightnesst(t *testing.T) {
	for _, test := range tests3 {
		if actual := GetTotalBrightness(test.input); actual != test.expected {
			t.Errorf("Convert(%s) = %d, expected %d.",
				test.input, actual, test.expected)
		}
	}
...
const day6data = `toggle 461,550 through 564,900
